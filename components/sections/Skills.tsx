  ...
  return (
    <Section id="skills">
      <div className="max-w-5xl mx-auto px-10 md:px-20">
        <div className="mb-12">
          <p className="text-xs text-white tracking-[0.4em] uppercase mb-3">02</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {displayGroups.map((group, i) => (
            ...
            >
              <p className="text-xs text-white tracking-[0.3em] uppercase mb-4">{group.category}</p>
              <div className="flex flex-wrap gap-2">
                {group.skills.map((skill) => (
                  <span
                    key={skill}
                    className="px-3 py-1.5 text-sm text-white border border-white/10 hover:border-white/30 transition-all duration-200"
                  >
                    {skill}
                  </span>
                ))}
