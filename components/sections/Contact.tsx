    ...
    <Section id="contact">
      <div className="max-w-5xl mx-auto px-10 md:px-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
          <div>
            <p className="text-base text-white leading-relaxed mb-8 max-w-sm">
            ...
            <div className="space-y-3">
              {links.map((link) => (
                <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer"
                  ...
                  <span className="text-sm text-white transition-colors">{link.label}</span>
                  <span className="text-white transition-colors text-xs">→</span>
                </a>
              ))}
            </div>
          </div>
          <div>
            {status === "sent" ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-xs text-white tracking-wider uppercase mb-2">Name</label>
                  <input type="text" required value={formState.name}
                    onChange={(e) => setFormState({ ...formState, name: e.target.value })}
                    className="w-full bg-transparent border border-white/10 text-white text-sm px-4 py-3 focus:outline-none focus:border-white/30 transition-colors placeholder:text-white"
                    placeholder="Your name" />
                </div>
                <div>
                  <label className="block text-xs text-white tracking-wider uppercase mb-2">Email</label>
                ...
                <div>
                  <label className="block text-xs text-white tracking-wider uppercase mb-2">Message</label>
                  <textarea required rows={5} value={formState.message}
                    onChange={(e) => setFormState({ ...formState, message: e.target.value })}
                    className="w-full bg-transparent border border-white/10 text-white text-sm px-4 py-3 focus:outline-none focus:border-white/30 transition-colors placeholder:text-white resize-none"
