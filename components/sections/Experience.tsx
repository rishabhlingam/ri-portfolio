  ...
  return (
    <Section id="experience">
      <div className="max-w-5xl mx-auto px-10 md:px-20">
        <div className="mb-12">
          <p className="text-xs text-white tracking-[0.4em] uppercase mb-3">03</p>
            ...
            >
              <div>
                <p className="text-xs text-white tracking-wide">
                  ...
                  {exp.current ? <span className="text-white">Present</span> : formatMonthYear(exp.endDate)}
                </p>
              </div>
              <div>
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    ...
                    <p className="text-sm text-white mt-0.5">{exp.company}</p>
                  </div>
                  {exp.current && (
                    <span className="mt-1 px-2 py-0.5 text-xs border border-white/20 text-white shrink-0">Current</span>
                  )}
                </div>
                <ul className="space-y-2 mb-4">
                  {(Array.isArray(exp.description) ? exp.description : [exp.description]).map((desc: string, j: number) => (
                    <li key={j} className="text-sm text-white pl-4 relative">
