            ...
            >
              {selected.image && (
                <div className="relative w-full max-h-[70vh] flex justify-center mb-6">
                  <Image
                    src={urlFor(selected.image).width(1200).height(800).url()}
                    alt={selected.title}
                    width={1200}
                    height={800}
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    {selected.location && (
                      <span className="text-xs text-white/35">{selected.location}</span>
                    )}
                    {selected.dateTaken && (
                      <span className="text-xs text-white/25">{formatDate(selected.dateTaken)}</span>
                    )}
                  </div>
                  {selected.description && (
                    <p className="text-sm text-white/35 mt-2 max-w-lg">{selected.description}</p>
                  )}
                </div>
                <button
                  onClick={() => setSelected(null)}
